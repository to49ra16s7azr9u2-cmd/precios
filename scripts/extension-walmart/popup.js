...
// la pestaña de la tienda y muestra lo que este responde. Así el recorrido
// sigue aunque el popup se cierre.
const PATRONES = ["https://www.walmart.com.mx/*", "https://www.bodegaaurrera.com.mx/*", "http://localhost/*"];
const RX = /walmart\.com\.mx|bodegaaurrera\.com\.mx|localhost/;
const $ = (id) => document.getElementById(id);
let pestaña = null;

async function pestañaDeLaTienda() {
  const [activa] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (activa && RX.test(activa.url || "")) return activa;
...
function pintar(r, extra) {
  ...
  if (!r) {
    e.className = "aviso";
    e.textContent = "No encuentro una pestaña de walmart.com.mx o bodegaaurrera.com.mx con un listado. Abre una búsqueda o un departamento y vuelve a abrir esto.";
  ...
  e.textContent = r.tienda + ": " + (r.dept || r.url.slice(0, 50)) +
...
document.addEventListener("DOMContentLoaded", async () => {
  pestaña = await pestañaDeLaTienda();
  await refrescar();
  ...
  $("lista").onclick = () => {
    const urls = $("urls").value.split(/\s+/).filter((u) => RX.test(u));
